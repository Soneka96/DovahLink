...
- The Dart SDK currently declares Host `0.4.x` as its supported range. Before `1.0.0`, the major and minor must match and the patch is ignored; after `1.0.0`, the major must match, a Host minor above the SDK's accepted minor is rejected, and the patch is ignored. The SDK checks `hello_ack.hostVersion` before session admission and closes on an incompatible Host.

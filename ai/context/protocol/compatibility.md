...
- This is the target contract, not yet fully built: SDK-side Host-version range declaration and enforcement are not implemented yet. Until `roadmap/05-dart-client-sdk-foundation.md`'s Stage 5 implements them, `hostVersion` is decoded and passed through without range validation, per `sdk/README.md`'s documented undone scope.

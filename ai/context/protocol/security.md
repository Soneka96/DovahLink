...
- Each trusted client receives a five-digit `shortId`, generated by the Bridge, unique among every
  ...
  transition, and is freed only once the record itself is removed (Forget or Factory Reset). A
  `shortId` is never authentication or authorization material and is never treated as
...
- bounded outbound queue: 128 messages per client, with 16 reserved control/recovery slots, 108
  ...
  the remaining ordered Event FIFO capacity. The registered-area count is bounded before publication

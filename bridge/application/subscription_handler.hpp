...
/// Builds the bridge capabilities envelope for an authenticated session. No capability is
/// currently registered (protocol/schema/README.md's "Registered state areas"), so the advertised
...
/// Validates a client capabilities envelope. No capability is currently registered, so any
...
struct SubscribeResult {
    ...
    /// Snapshots for accepted state areas, in request order. Always empty: no state area is
    /// currently registered.
    std::vector<protocol::Envelope> snapshots;

    /// State areas accepted by this request, exposed structurally (beyond `subscriptionAck`'s
    /// encoded payload) for the dispatcher's own per-connection subscription bookkeeping. Always
    /// empty: no state area is currently registered.
    std::vector<std::string> acceptedStateAreas;
};

/// Handles a subscription request. No state area is currently registered
/// (protocol/schema/README.md's "Registered state areas"), so every requested area is rejected
/// into the acknowledgement's `rejectedStateAreas` and no snapshot is ever produced.
...
/// Handles a request for a fresh state snapshot. No state area is currently registered, so every
/// request is rejected as `unsupported_capability`.
/// @param snapshotRequestEnvelope Decoded client snapshot request.
/// @param sessionId Authenticated session identifier.
/// @return An `unsupported_capability` error envelope, or `malformed_message` for an invalid payload.

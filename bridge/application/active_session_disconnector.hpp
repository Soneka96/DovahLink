...
public:
    ...
    ///
    /// An implementation must resolve the target identity and capture the specific socket handle to
    /// close together under one critical section, then act only on that captured handle rather than
    /// re-resolving "whoever is active" once the section ends -- otherwise a session change landing
    /// between resolution and shutdown could close a different, newly-connected client's socket
    /// instead of the one actually identified. `BridgeWorkerPool` satisfies this by capturing the
    /// socket handle as a `shared_ptr` under `activeSocketMutex_` before releasing it, so a later
    /// reassignment of the active socket cannot redirect an already-captured shutdown.
    ...
    /// is active.
    ///
    /// Known limitation: a connection accepted moments earlier but not yet past its own WebSocket
    /// handshake (still pre-`hello`) may not be reliably interrupted by one call alone -- unlike a
    /// full pool `Stop()`, this does not set a pool-wide stopping flag the accept loop's own lambda
    /// also checks as a second safety net once it publishes the socket, so a call landing in that
    /// narrow pre-publication window has nothing left to cancel. Accepted because this method's real

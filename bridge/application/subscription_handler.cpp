...
HandleClientCapabilities(const protocol::Envelope& capabilitiesEnvelope,
    ...
    auto capabilities =
        protocol::DecodeCapabilitiesPayload(capabilitiesEnvelope.payload);
    if (!capabilities.has_value()) {
        return protocol::BuildErrorEnvelope(
            capabilitiesEnvelope.messageId, sessionId, "malformed_message",
    ...
    if (!capabilities->capabilities.empty()) {
        return protocol::BuildErrorEnvelope(
            ...
            "Unsupported capability: " + capabilities->capabilities.front().id,
            false);
    }
    return std::nullopt;
}

SubscribeResult HandleSubscribe(const protocol::Envelope& subscribeEnvelope,
                                const std::string& sessionId) {
    auto subscribe = protocol::DecodeSubscribePayload(subscribeEnvelope.payload);
    if (!subscribe.has_value()) {
        return SubscribeResult{
            .subscriptionAck = protocol::BuildErrorEnvelope(
                subscribeEnvelope.messageId, sessionId, "malformed_message",
                "Malformed subscribe payload", false),
        };
    }

    //  No state area is currently registered (protocol/schema/README.md's
    //  "Registered state areas"), so every requested area is rejected.
    //  Deduplicated the same way an accepted area would be, so a repeated area is
    //  not reported twice.
    ...
    auto ackEnvelope = protocol::BuildEnvelope(
        std::string(protocol::message_type::kSubscriptionAck), sessionId,
    ...
    if (!ackEnvelope.has_value()) {
        return SubscribeResult{
            .subscriptionAck = protocol::BuildErrorEnvelope(
                subscribeEnvelope.messageId, sessionId, "internal_error",
...
HandleSnapshotRequest(const protocol::Envelope& snapshotRequestEnvelope,
                      const std::string& sessionId) {
    auto request =
        protocol::DecodeSnapshotRequestPayload(snapshotRequestEnvelope.payload);
    if (!request.has_value()) {
        return protocol::BuildErrorEnvelope(
            snapshotRequestEnvelope.messageId, sessionId, "malformed_message",
            "Malformed snapshot_request payload", false);
    }
    //  No state area is currently registered (protocol/schema/README.md's
    //  "Registered state areas"), so every request is rejected.
    return protocol::BuildErrorEnvelope(
        snapshotRequestEnvelope.messageId, sessionId, "unsupported_capability",

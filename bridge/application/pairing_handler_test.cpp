...
///  `ITrustStorePersistence` double that always loads an empty snapshot but
///  fails every `Save`, for exercising `TrustStore::Persist` failure.
...
TEST_CASE("full pairing flow: request, confirm, ack results in a trusted, "
          "upgraded session",
    ...
    auto statusResponse =
        HandlePairingRequest(BuildPairingRequestEnvelope(), kSessionId, kClientId,
                             pairingSession, sink, now);
    REQUIRE(statusResponse.messageType == "pairing_status");
    auto status =
        dovahlink::protocol::DecodePairingStatusPayload(statusResponse.payload);
    ...
    auto confirmResponse = HandlePairingConfirm(
        BuildPairingConfirmEnvelope("123456", std::string("My PC")), kSessionId,
        kClientId, pairingSession, sink, now);
    ...
    auto ackResponse = HandlePairingAck(
        BuildPairingAckEnvelope(credentialHex), kSessionId, kClientId,
...
TEST_CASE("a second pairing_request from the same client reports in_progress "
          "with a live "
    ...
    auto first = HandlePairingRequest(BuildPairingRequestEnvelope(), kSessionId,
                                      kClientId, pairingSession, sink, now);
    auto firstStatus =
        dovahlink::protocol::DecodePairingStatusPayload(first.payload);
    ...
    auto secondStatus =
        dovahlink::protocol::DecodePairingStatusPayload(second.payload);
...
TEST_CASE("a pairing_request from the same client reports in_progress with "
          "null expiresInSeconds "
          "once its own code is already confirmed and pending acknowledgement",
          "[application][pairing_handler]") {
    //  Pins protocol/schema/README.md's pairing_status contract: expiresInSeconds
    //  tracks the *code's* own remaining validity, not "how long until this state
    //  changes" in general -- once the owner's code is consumed by a successful
    //  pairing_confirm (PENDING_CREDENTIAL), a repeated pairing_request still
    //  resumes as "in_progress" (the client still owns something to resume), but
    //  there is no code left to count down, so expiresInSeconds is null (present,
    //  not omitted -- omission is reserved for other_device_pairing).
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    auto now = std::chrono::steady_clock::now();
    ...
    auto requested =
        HandlePairingRequest(BuildPairingRequestEnvelope(), kSessionId, kClientId,
                             pairingSession, sink, now);
    REQUIRE(dovahlink::protocol::DecodePairingStatusPayload(requested.payload)
                ->state == "available");

    auto confirmed =
        HandlePairingConfirm(BuildPairingConfirmEnvelope("123456"), kSessionId,
                             kClientId, pairingSession, sink, now);
    auto confirmedOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(confirmed.payload);
    ...
    auto resumed =
        HandlePairingRequest(BuildPairingRequestEnvelope("message-request-2"),
                             kSessionId, kClientId, pairingSession, sink, now);
    auto resumedStatus =
        dovahlink::protocol::DecodePairingStatusPayload(resumed.payload);
    REQUIRE(resumedStatus.has_value());
    CHECK(resumedStatus->state == "in_progress");
    CHECK_FALSE(resumedStatus->expiresInSeconds.has_value());
}

TEST_CASE("a pairing_request from a different client reports "
          "other_device_pairing and reveals no "
          "expiresInSeconds",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto owner = HandlePairingRequest(BuildPairingRequestEnvelope(), kSessionId,
                                      kClientId, pairingSession, sink, now);
    REQUIRE(
        dovahlink::protocol::DecodePairingStatusPayload(owner.payload)->state ==
        "available");

    auto competing = HandlePairingRequest(
        BuildPairingRequestEnvelope("message-request-2"), kSessionId,
        kOtherClientId, pairingSession, sink, now);
    auto competingStatus =
        dovahlink::protocol::DecodePairingStatusPayload(competing.payload);
...
TEST_CASE("HandlePairingRequest reports unavailable when the code generator "
          "fails, distinct from "
          "in_progress",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
    ...
    auto status =
        dovahlink::protocol::DecodePairingStatusPayload(response.payload);
    REQUIRE(status.has_value());
    CHECK(status->state == "unavailable");
    CHECK_FALSE(status->expiresInSeconds.has_value());
    //  Nothing was generated to display -- the sink must never be notified with no
    //  code.
    CHECK(sink.codes.empty());
}

TEST_CASE("HandlePairingConfirm reports expired for an expired code",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); },
    ...
    auto outcome =
        dovahlink::protocol::DecodePairingOutcomePayload(response.payload);
...
TEST_CASE("HandlePairingConfirm reports invalid for a wrong code and "
          "redisplays it as incorrect",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    ...
    auto response =
        HandlePairingConfirm(BuildPairingConfirmEnvelope("000000"), kSessionId,
                             kClientId, pairingSession, sink, now);
...
TEST_CASE("HandlePairingConfirm reports pacing_limited for an attempt "
          "evaluated too soon after the "
    ...
    static_cast<void>(HandlePairingRequest(BuildPairingRequestEnvelope(),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));
    static_cast<void>(HandlePairingConfirm(BuildPairingConfirmEnvelope("000000"),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));
    ...
    //  The paced-out attempt is never evaluated, so it is never redisplayed
    //  either.
    CHECK(sink.incorrectCodes.size() == 1);
}

TEST_CASE("HandlePairingConfirm reports hard_limit_reached on the fifth wrong "
          "evaluated attempt "
          "and displays a distinct exhausted notification instead of "
          "redisplaying the code",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        static_cast<void>(
            HandlePairingConfirm(BuildPairingConfirmEnvelope("000000"), kSessionId,
                                 kClientId, pairingSession, sink, attemptTime));
    }
    auto response = HandlePairingConfirm(BuildPairingConfirmEnvelope("000000"),
                                         kSessionId, kClientId, pairingSession,
    ...
    //  Only the first wrong attempt redisplays -- the next three land inside its
    //  5-second auto-renotify cooldown, and the fifth (the hard-limit attempt
    //  itself) uses the distinct exhausted notification instead of the ordinary
    //  incorrect-code path.
    CHECK(sink.incorrectCodes.size() == 1);
}

TEST_CASE("a non-owning client's wrong-code confirm reports invalid and "
    ...
    //  kOtherClientId does not own the active challenge -- even the genuinely
    //  correct code is rejected as though it were wrong, and no notification of
    //  any kind fires for it.
    auto response =
        HandlePairingConfirm(BuildPairingConfirmEnvelope("123456"), kSessionId,
                             kOtherClientId, pairingSession, sink, now);
    ...
    //  The real owner's correct code still works afterward -- the non-owner's
...
TEST_CASE("a fresh pairing_request succeeds immediately after "
          "hard_limit_reached cancels the "
          "previous challenge",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingRequest(
        BuildPairingRequestEnvelope("message-request-2"), kSessionId, kClientId,
...
TEST_CASE("HandlePairingAck reports pending_not_found once the pending "
          "credential's 5-minute TTL "
          "has elapsed",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingAck(
        BuildPairingAckEnvelope(credentialHex), kSessionId, kClientId,
        kConnection, pairingSession, trustStore, sessions, pastTtl);
...
TEST_CASE("HandlePairingConfirm is rejected as malformed when the code field "
          "is missing",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingConfirm(confirmEnvelope, kSessionId, kClientId,
                                         pairingSession, sink,
...
TEST_CASE(
    "HandlePairingAck reports pending_not_found for a mismatched credential",
    "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    EmptyPersistence persistence;
    ...
    static_cast<void>(HandlePairingRequest(BuildPairingRequestEnvelope(),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));
    static_cast<void>(HandlePairingConfirm(BuildPairingConfirmEnvelope("123456"),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));

    auto response = HandlePairingAck(
        BuildPairingAckEnvelope(EncodeHex(std::vector<std::uint8_t>{9, 9, 9, 9})),
...
TEST_CASE("HandlePairingAck reports pending_not_found with no pending "
          "credential (simulating a "
          "Bridge restart)",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingAck(
        BuildPairingAckEnvelope(EncodeHex(std::vector<std::uint8_t>{1, 2, 3, 4})),
        kSessionId, kClientId, kConnection, pairingSession, trustStore, sessions,
        std::chrono::steady_clock::now());
...
TEST_CASE("HandlePairingAck reports pending_not_found and persists nothing "
          "when an administrative "
          "CancelAll (e.g. Reset Trust) consumed the pending credential first",
          "[application][pairing_handler]") {
    //  Regression coverage for the pending-credential race:
    //  PairingSession::CancelAll and PairingSession::CommitPending share one
    //  mutex, so whichever runs first wins deterministically. This proves the
    //  CancelAll-first outcome; the CommitPending-first outcome is already covered
    //  by every successful-ack test above. Before the CommitPending-before-Persist
    //  reordering in pairing_handler.cpp, TrustStore::Persist ran off a value
    //  already captured by PeekPending and never re-checked PairingSession's
    //  state, so a CancelAll landing in this same window could not stop the
    //  credential from being persisted -- this test would have failed against that
    //  old order.
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    EmptyPersistence persistence;
    ...
    //  Simulates an administrative Reset Trust/Factory Reset's CancelAll winning
    //  the race against this pairing_ack, landing between pairing_confirm (above)
    //  and pairing_ack (below).
    pairingSession.CancelAll();
...
TEST_CASE("HandlePairingAck reports already_trusted on a retry after the "
    ...
    auto firstAck = HandlePairingAck(BuildPairingAckEnvelope(credentialHex),
                                     kSessionId, kClientId, kConnection,
                                     pairingSession, trustStore, sessions, now);
    auto firstOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(firstAck.payload);
    REQUIRE(firstOutcome.has_value());
    REQUIRE(firstOutcome->outcome == "trusted");
    REQUIRE(firstOutcome->shortId.has_value());

    //  Simulates a lost success response: the client resends pairing_ack with the
    //  same credential.
    auto retryAck = HandlePairingAck(BuildPairingAckEnvelope(credentialHex),
                                     kSessionId, kClientId, kConnection,
                                     pairingSession, trustStore, sessions, now);
    auto retryOutcome =
...
TEST_CASE("HandlePairingAck reports internal_error when TrustStore::Persist "
          "fails to save",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
...
TEST_CASE("HandlePairingAck supports an explicit pairing restart after a persist "
    ...
    static_cast<void>(
        HandlePairingRequest(BuildPairingRequestEnvelope("message-request-2"),
                             kSessionId, kClientId, pairingSession, sink, now));
    auto secondConfirmResponse = HandlePairingConfirm(
        BuildPairingConfirmEnvelope("123456", std::nullopt, "message-confirm-2"),
        kSessionId, kClientId, pairingSession, sink, now);
    auto secondConfirmOutcome = dovahlink::protocol::DecodePairingOutcomePayload(
        secondConfirmResponse.payload);
    REQUIRE(secondConfirmOutcome.has_value());
    std::string secondCredentialHex = *secondConfirmOutcome->credential;

    //  This time the underlying store's simulated failure is already spent, so
    //  persist succeeds.
    auto secondAck = HandlePairingAck(
        BuildPairingAckEnvelope(secondCredentialHex, "message-ack-2"), kSessionId,
        kClientId, kConnection, pairingSession, trustStore, sessions, now);
    auto secondAckOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(secondAck.payload);
    REQUIRE(secondAckOutcome.has_value());
    CHECK(secondAckOutcome->outcome == "trusted");
    CHECK(sessions.IsFullyTrusted(kConnection));
    auto record = trustStore.Query(kClientId);
    REQUIRE(record.has_value());
    CHECK(EncodeHex(record->credential) == secondCredentialHex);
}
...
TEST_CASE("two separate successful pairing flows issue different credentials",
    ...
    auto firstConfirm =
        HandlePairingConfirm(BuildPairingConfirmEnvelope("123456"), kSessionId,
                             kClientId, pairingSession, sink, now);
    auto firstOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(firstConfirm.payload);
    REQUIRE(firstOutcome.has_value());
    ...
    auto sessionLease = sessions.TryCreateSession(
        ...
        SessionAuthMethod::kUnpaired);
    REQUIRE(sessionLease.has_value());
    static_cast<void>(HandlePairingAck(
        BuildPairingAckEnvelope(*firstOutcome->credential), kSessionId, kClientId,
        kConnection, pairingSession, trustStore, sessions, now));

    static_cast<void>(
        HandlePairingRequest(BuildPairingRequestEnvelope("message-request-2"),
                             kSessionId, kClientId, pairingSession, sink, now));
    auto secondConfirm = HandlePairingConfirm(
        BuildPairingConfirmEnvelope("123456", std::nullopt, "message-confirm-2"),
        kSessionId, kClientId, pairingSession, sink, now);
    auto secondOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(secondConfirm.payload);
...
TEST_CASE("HandlePairingAck does not upgrade to full trust when the presented "
          "clientId collides "
    ...
    auto trustStore = TrustStore::Load(persistence);
    REQUIRE(trustStore
                .Persist(kClientId, std::vector<std::uint8_t>{1, 2, 3, 4},
                         std::nullopt)
                .has_value());
    SessionManager sessions;
    auto sessionLease = sessions.TryCreateSession(
    ...
    //  No pairing_confirm ever ran on this session -- an unpaired connection
    //  simply claiming the same clientId and guessing at a credential.
    auto response = HandlePairingAck(
        BuildPairingAckEnvelope(EncodeHex(std::vector<std::uint8_t>{9, 9, 9, 9})),
        kSessionId, kClientId, kConnection, pairingSession, trustStore, sessions,
        std::chrono::steady_clock::now());
...
TEST_CASE("HandlePairingAck is rejected as malformed when the credential field "
          "is missing",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingAck(ackEnvelope, kSessionId, kClientId,
                                     kConnection, pairingSession, trustStore,
...
TEST_CASE(
    "HandlePairingRenotify redisplays the owner's code and reports renotified",
    "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    ...
    auto response =
        HandlePairingRenotify(BuildPairingRenotifyEnvelope(), kSessionId,
                              kClientId, pairingSession, sink, now);
...
TEST_CASE("HandlePairingRenotify reports renotify_cooldown with no redisplay "
          "while its own cooldown "
          "is active",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    static_cast<void>(HandlePairingRequest(BuildPairingRequestEnvelope(),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));
    static_cast<void>(HandlePairingRenotify(BuildPairingRenotifyEnvelope(),
                                            kSessionId, kClientId, pairingSession,
                                            sink, now));

    auto response = HandlePairingRenotify(
        BuildPairingRenotifyEnvelope("message-renotify-2"), kSessionId, kClientId,
...
TEST_CASE("HandlePairingRenotify reports already_idle for a client owning no "
          "challenge",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    ...
    auto response = HandlePairingRenotify(BuildPairingRenotifyEnvelope(),
                                          kSessionId, kClientId, pairingSession,
                                          sink, std::chrono::steady_clock::now());
...
TEST_CASE("HandlePairingRenotify succeeds again once its cooldown elapses",
    ...
    REQUIRE(outcome.has_value());
    CHECK(outcome->outcome == "renotified");
    //  The original display, plus one redisplay per successful renotify -- both
    //  the first (at `now`) and this second one (at `now + 5s`, exactly when its
    //  cooldown elapses) succeeded.
    REQUIRE(sink.codes.size() == 3);
    CHECK(sink.codes[1] == "123456");
    CHECK(sink.codes[2] == "123456");
}

TEST_CASE("HandlePairingRenotify reports already_idle once the challenge "
          "reaches PENDING_CREDENTIAL",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    ...
    REQUIRE(outcome.has_value());
    CHECK(outcome->outcome == "already_idle");
    //  Only the original display -- a pending credential has no code left to
    //  redisplay.
    CHECK(sink.codes.size() == 1);
}

TEST_CASE("HandlePairingRenotify reports already_idle for a non-owner without "
          "disturbing the "
    ...
    REQUIRE(outcome.has_value());
    CHECK(outcome->outcome == "already_idle");
    //  Only the original display -- the non-owner's request never redisplayed
    //  anything.
    CHECK(sink.codes.size() == 1);

    auto ownerResponse =
        HandlePairingRenotify(BuildPairingRenotifyEnvelope("message-renotify-2"),
                              kSessionId, kClientId, pairingSession, sink, now);
    auto ownerOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(ownerResponse.payload);
    REQUIRE(ownerOutcome.has_value());
...
TEST_CASE("HandlePairingCancel clears an owned active challenge and frees it "
          "for a fresh "
          "pairing_request",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
    ...
    auto response = HandlePairingCancel(BuildPairingCancelEnvelope(), kSessionId,
                                        kClientId, pairingSession, now);
    ...
    auto fresh =
        HandlePairingRequest(BuildPairingRequestEnvelope("message-request-2"),
                             kSessionId, kClientId, pairingSession, sink, now);
    auto freshStatus =
        dovahlink::protocol::DecodePairingStatusPayload(fresh.payload);
    REQUIRE(freshStatus.has_value());
    CHECK(freshStatus->state == "available");
}

TEST_CASE("HandlePairingCancel clears an owned pending credential, leaving it "
          "unreachable by a "
          "later pairing_ack",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
...
TEST_CASE("HandlePairingCancel is idempotent: repeating it reports "
          "already_idle truthfully",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
        []() -> std::optional<std::string> { return std::string("123456"); });
    RecordingPairingNotificationSink sink;
    ...
    static_cast<void>(HandlePairingRequest(BuildPairingRequestEnvelope(),
                                           kSessionId, kClientId, pairingSession,
                                           sink, now));
    auto first = HandlePairingCancel(BuildPairingCancelEnvelope(), kSessionId,
                                     kClientId, pairingSession, now);
    auto firstOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(first.payload);
    REQUIRE(firstOutcome.has_value());
    REQUIRE(firstOutcome->outcome == "cancelled");

    auto second =
        HandlePairingCancel(BuildPairingCancelEnvelope("message-cancel-2"),
                            kSessionId, kClientId, pairingSession, now);

    auto secondOutcome =
        dovahlink::protocol::DecodePairingOutcomePayload(second.payload);
    REQUIRE(secondOutcome.has_value());
...
TEST_CASE("HandlePairingCancel reports already_idle with nothing owned",
    ...
    auto response =
        HandlePairingCancel(BuildPairingCancelEnvelope(), kSessionId, kClientId,
...
TEST_CASE("HandlePairingCancel reports already_idle for a non-owner without "
          "disturbing the real "
          "owner's challenge",
          "[application][pairing_handler]") {
    PairingSession pairingSession(
    ...
    auto response = HandlePairingCancel(BuildPairingCancelEnvelope(), kSessionId,
                                        kOtherClientId, pairingSession, now);
    ...
    //  The real owner's challenge survives the non-owner's cancel attempt
    //  untouched.
    auto ownerResponse =
        HandlePairingConfirm(BuildPairingConfirmEnvelope("123456"), kSessionId,
                             kClientId, pairingSession, sink, now);

...
TEST_CASE("FormatTimestamp matches the canonical fixtures' example value",
...
TEST_CASE("FormatTimestamp zero-pads single-digit month, day, hour, minute, "
          "and second",
          "[application][timestamp]") {
  auto time =
      sys_days{2026y / January / 5} + hours(3) + minutes(4) + seconds(9);
...
TEST_CASE("FormatTimestamp handles a year boundary",
          "[application][timestamp]") {
  auto time =
      sys_days{2025y / December / 31} + hours(23) + minutes(59) + seconds(59);

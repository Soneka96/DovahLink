...
TEST_CASE("BuildBridgeCapabilities advertises an empty capability list", "[application][subscription_handler]") {
...
TEST_CASE("HandleClientCapabilities rejects any non-empty capability list", "[application][subscription_handler]") {
...
TEST_CASE("HandleClientCapabilities reports only the first entry when multiple are present",
          "[application][subscription_handler]") {
    auto envelope = BuildEnvelopeWithPayload(
        "capabilities",
...
TEST_CASE("HandleSubscribe rejects every requested state area, producing no snapshots",
          "[application][subscription_handler]") {
    auto envelope = BuildEnvelopeWithPayload("subscribe", R"({"stateAreas": ["example_area"]})", "message-sub-1");
...
TEST_CASE("HandleSubscribe rejects multiple requested state areas, each exactly once",
          "[application][subscription_handler]") {
    auto envelope =
        BuildEnvelopeWithPayload("subscribe", R"({"stateAreas": ["example_area", "other_area"]})");
    auto result = HandleSubscribe(envelope, kSessionId);

    auto ack = dovahlink::protocol::DecodeSubscriptionAckPayload(result.subscriptionAck.payload);
    REQUIRE(ack.has_value());
    CHECK(ack->acceptedStateAreas.empty());
    CHECK(ack->rejectedStateAreas == std::vector<std::string>{"example_area", "other_area"});
}

TEST_CASE("HandleSubscribe deduplicates a repeated state area in the rejection list",
          "[application][subscription_handler]") {
    auto envelope =
        BuildEnvelopeWithPayload("subscribe", R"({"stateAreas": ["example_area", "example_area"]})");
    auto result = HandleSubscribe(envelope, kSessionId);
...
TEST_CASE("HandleSubscribe deduplicates a non-adjacent repeat, preserving first-occurrence order",
          "[application][subscription_handler]") {
    auto envelope =
        BuildEnvelopeWithPayload("subscribe", R"({"stateAreas": ["area_a", "area_b", "area_a"]})");
    auto result = HandleSubscribe(envelope, kSessionId);

    auto ack = dovahlink::protocol::DecodeSubscriptionAckPayload(result.subscriptionAck.payload);
    REQUIRE(ack.has_value());
    CHECK(ack->rejectedStateAreas == std::vector<std::string>{"area_a", "area_b"});
}

TEST_CASE("HandleSubscribe accepts an empty stateAreas list as a no-op unsubscribe",
          "[application][subscription_handler]") {
    auto envelope = BuildEnvelopeWithPayload("subscribe", R"({"stateAreas": []})");
    auto result = HandleSubscribe(envelope, kSessionId);
...
TEST_CASE("HandleSnapshotRequest rejects every state area as unsupported_capability",
          "[application][subscription_handler]") {
    auto envelope = BuildEnvelopeWithPayload("snapshot_request", R"({"stateArea": "example_area"})");

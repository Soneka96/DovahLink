...
//    `unauthenticated` when TrustStore::IsRevoked reports true for the presented
//    clientId: revocation is a bridge-side decision the client did not cause,
//    and the distinct code lets the app return the user directly to pairing
...
//    Device blocking (security.md's "Developer authentication"). This check
//    touches no throttle or trust-store mutation state, so a blocked attempt
//    updates no persisted metadata, matching the acceptance criterion.
...
HandshakeResult Fail(const protocol::Envelope& helloEnvelope,
    ...
    protocol::Envelope response = protocol::BuildErrorEnvelope(
        helloEnvelope.messageId, /*sessionId=*/std::nullopt, std::move(code),
...
HandshakeResult HandleHello(const protocol::Envelope& helloEnvelope,
    ...
    //  A structurally invalid presented credential (not valid hex) can never match
    //  a stored one; treat it as an immediate failed attempt without a store
    //  lookup, matching this codebase's existing accepted precedent for Phase 1's
    //  timing-side-channel posture (see token_store.cpp's known-limitation comment
    //  on TryReserve). Shared by both credentialed methods below; "unpaired" has
    //  no credential to decode at all.
    auto presentedBytes = [&hello]() -> std::optional<std::vector<std::uint8_t>> {
        return hello->authToken.has_value()
                   ? security::DecodeHex(*hello->authToken)
    ...
    if (hello->authMethod == "one_time_local_token") {
        ...
        //  No credential to present or check yet; the session is admitted
        //  restricted, per security.md's "Hello authentication and session trust
        //  tiers".
    ...
    if (!sessionId.has_value() || !responseMessageId.has_value()) {
        return Fail(helloEnvelope, bridgeInstanceId, "internal_error",
                    "Unable to establish a secure connection", false);
    }

    //  "paired" only for a session admitted with an already-persisted trust
    //  credential; a developer-authenticated or still-unpaired session reports
    //  "unpaired" regardless of trust tier (security.md: developer authentication
    //  is never wire-visible as "paired").
    ...
    //  sessionLease going out of scope on the return below invalidates the session
    ...
    if (auto trustLoss =
        ...
        return Fail(helloEnvelope, bridgeInstanceId, std::string(*trustLoss),
...
TrustLossAfterAdmission(security::ITrustStore& trustStore,
    ...
    if (authMethod == SessionAuthMethod::kTrustedDeviceCredential &&
        trustStore.IsRevoked(clientId)) {
        return "revoked";
    }
    return std::nullopt;
}

...
/// A complete, self-consistent snapshot of one active authenticated session, returned by value --
/// never a reference into `SessionManager`'s own mutable state -- so a caller inspects every field
/// together without further locking or any risk of reading a partially-updated record.

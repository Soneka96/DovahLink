...
std::string SnapshotRequestMessage(std::string messageId = "message-snap-req-1") {
    return R"({"messageType": "snapshot_request", "messageId": ")" + messageId + R"(", "sessionId": ")" +
           kSessionId + R"(", "correlationId": null, "payload": {"stateArea": "example_area"}, )"
           R"("bridgeInstanceId": null, "playContextId": null, "clientId": null})";
}
...
TEST_CASE("ProcessInboundMessage routes subscribe to a subscription_ack that rejects every "
          "requested area",
          "[application][message_dispatcher]") {
    // No state area is currently registered (protocol/schema/README.md's "Registered state
    // areas"), so subscribe never produces a snapshot.
    Fixture fixture(SessionTrustTier::kFull, SessionAuthMethod::kTrustedDeviceCredential);
    ...
    auto ack = dovahlink::protocol::DecodeSubscriptionAckPayload(result.responses[0].payload);
...
TEST_CASE("ProcessInboundMessage routes snapshot_request to an unsupported_capability error",
          "[application][message_dispatcher]") {
    // No state area is currently registered (protocol/schema/README.md's "Registered state
    // areas"), so every snapshot_request is rejected.
    Fixture fixture(SessionTrustTier::kFull, SessionAuthMethod::kTrustedDeviceCredential);
...
TEST_CASE("ProcessInboundMessage's capabilities handling is unaffected by play-context routing",
          "[application][message_dispatcher]") {
    // HandleClientCapabilities never touches the play context, so it must behave identically
    // whether or not one is active -- proven explicitly here rather than left as an unstated
    // assumption.
    Fixture fixture(SessionTrustTier::kFull, SessionAuthMethod::kTrustedDeviceCredential);
    fixture.activePlayContext.Begin("context-1");
    std::string message =
...
TEST_CASE("ProcessInboundMessage accepts an empty capabilities list on a Restricted session too",
          "[application][message_dispatcher]") {
    // capabilities is in kRestrictedAllowedMessageTypes -- the only one of the two capabilities
    // tests above proven on a Restricted session, matching the "lets a Restricted session
    // exchange pairing messages but rejects subscribe/snapshot_request" test's own tier scope.
    Fixture fixture(SessionTrustTier::kRestricted, SessionAuthMethod::kUnpaired);
    std::string message =
        R"({"messageType": "capabilities", "messageId": "message-cap-1", "sessionId": ")" +
        std::string(kSessionId) + R"(", "correlationId": null, "payload": {"capabilities": []}, )"

...
std::int64_t RevisionTracker::NextRevisionLocked(
    ...
    if (fingerprint.has_value() && it->second.second.has_value() &&
        *fingerprint == *it->second.second) {
...
std::int64_t
RevisionTracker::StartSnapshot(const std::string& stateArea,
...
std::int64_t RevisionTracker::NextSnapshotRevision(
    const std::string& stateArea,
    const std::optional<std::string>& fingerprint) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
RevisionTracker::NextEvent(const std::string& stateArea) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = currentRevision_.find(stateArea);
    if (it == currentRevision_.end()) {
        return std::nullopt;
...
RevisionTracker::CurrentRevision(const std::string& stateArea) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = currentRevision_.find(stateArea);
    if (it == currentRevision_.end()) {
        return std::nullopt;

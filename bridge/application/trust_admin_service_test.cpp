...
///  `ActiveSessionDisconnector` double recording every call it receives, so
///  tests can assert exactly which clientId (if any) and typed reason a
///  successful revoke, block, or reset disconnected with. Matches this project's
...
class RecordingSessionDisconnector : public ActiveSessionDisconnector {
  public:
    ///  Appends `clientId` to `disconnectedClientIds` and `reason` to
    ///  `disconnectReasons`.
    ...
    ///  Increments `disconnectActiveCallCount` and appends `reason` to
    ///  `disconnectReasons`.
    ...
    ///  Every reason passed to either `DisconnectIfClientActive` or
    ///  `DisconnectActive`, in order.
    std::vector<std::string> disconnectReasons;
};

///  In-memory `ITrustStorePersistence` double: never touches real storage, and
///  can be configured to report a failing save. Matches this project's per-file
///  test-double convention (see `security/trust_store_test.cpp`'s own
///  `FakePersistence`).
...
///  Builds a known-device record with a deterministic creation time for ordering
///  assertions.
...
QueuedFactoryResetCodes(std::deque<std::string> codes) {
...
TEST_CASE("ListTrusted reports no trusted clients on an empty store",
...
TEST_CASE("ListTrusted formats one client with a display name, using singular "
          "phrasing",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(store.Persist("client-1", MakeCredential(1), std::string("My Phone"))
                .has_value());
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
    FactoryResetChallenge factoryResetChallenge;
...
TEST_CASE("ListTrusted shows a placeholder for an absent display name",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
...
TEST_CASE("ListTrusted uses plural phrasing and lists every client",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store =
        TrustStore::Load(persistence, QueuedShortIds({"11111", "22222"}));
    REQUIRE(store.Persist("client-1", MakeCredential(1), std::string("Phone"))
                .has_value());
    REQUIRE(store.Persist("client-2", MakeCredential(2), std::string("Tablet"))
                .has_value());
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
    FactoryResetChallenge factoryResetChallenge;
...
TEST_CASE("ListTrusted disambiguates duplicate display names oldest first",
    ...
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-2", "22222", std::string("Shared"),
                            KnownDeviceState::kTrusted, 100),
            MakeKnownDevice("client-1", "11111", std::string("Shared"),
                            KnownDeviceState::kTrusted, 100),
        }});
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    ...
    CHECK(service.ListTrusted() ==
          "2 trusted clients:\n11111  Shared #1\n22222  Shared #2");
}

TEST_CASE("List scopes select all trusted and blocked devices",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-1", "11111", std::string("Trusted"),
                            KnownDeviceState::kTrusted, 100),
            MakeKnownDevice("client-2", "22222", std::string("Revoked"),
                            KnownDeviceState::kRevoked, 200),
            MakeKnownDevice("client-3", "33333", std::string("Blocked"),
                            KnownDeviceState::kBlocked, 300),
        }});
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
...
TEST_CASE("List rejects an unknown scope with the canonical choices",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    ...
    CHECK(service.List("revoked") ==
          "Unknown list scope 'revoked'. Use all, trust, or block.");
}

TEST_CASE("Help lists only the canonical trust-admin commands",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE("RevokeByShortId reports not-found on an empty store",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE("ListKnownDevices formats a single device with its state",
    ...
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-1", "11111", std::string("Phone"),
                            KnownDeviceState::kTrusted, 100),
        }});
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE("ListKnownDevices keeps distinct display names unsuffixed and sorts "
          "by creation time",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-2", "22222", std::string("Tablet"),
                            KnownDeviceState::kRevoked, 200),
            MakeKnownDevice("client-1", "11111", std::string("Phone"),
                            KnownDeviceState::kTrusted, 100),
        }});
...
TEST_CASE(
    "ListKnownDevices disambiguates two and three duplicate names oldest first",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-3", "33333", std::string("Shared"),
                            KnownDeviceState::kUnpaired, 300),
            MakeKnownDevice("client-1", "11111", std::string("Shared"),
                            KnownDeviceState::kRevoked, 100),
            MakeKnownDevice("client-2", "22222", std::string("Shared"),
                            KnownDeviceState::kBlocked, 200),
        }});
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
...
TEST_CASE("ListKnownDevices recomputes duplicate suffixes after forgetting the "
          "oldest device",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
    ...
    REQUIRE(service.ListKnownDevices().find("11111  Shared #1  revoked") !=
            std::string::npos);
    REQUIRE(service.ListKnownDevices().find("22222  Shared #2  revoked") !=
            std::string::npos);
    CHECK(service.ForgetByShortId("11111") == "Forgot device 11111 (Shared).");
    CHECK(service.ListKnownDevices() ==
          "1 known device:\n22222  Shared  revoked");
}

TEST_CASE(
    "ListBlocked includes blocked devices while excluding nonblocked devices",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(TrustStoreSnapshot{
        .devices = {
            MakeKnownDevice("client-1", "11111", std::string("Phone"),
                            KnownDeviceState::kTrusted, 100),
            MakeKnownDevice("client-2", "22222", std::string("Blocked Phone"),
                            KnownDeviceState::kBlocked, 200),
            MakeKnownDevice("client-3", "33333", std::nullopt,
                            KnownDeviceState::kRevoked, 300),
            MakeKnownDevice("client-4", "44444", std::string("Blocked Phone"),
                            KnownDeviceState::kBlocked, 150),
        }});
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
    ...
    CHECK(blocked == "2 blocked devices:\n44444  Blocked Phone #1  "
                     "blocked\n22222  Blocked Phone #2  blocked");
}

TEST_CASE("RevokeByShortId reports not-found for an unknown shortId without "
          "changing the store",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE(
    "RevokeByShortId revokes the matching client and reports its display name",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(store.Persist("client-1", MakeCredential(1), std::string("Phone"))
    ...
    CHECK(disconnector.disconnectedClientIds ==
          std::vector<std::string>{"client-1"});
    CHECK(disconnector.disconnectActiveCallCount == 0);
    CHECK(disconnector.disconnectReasons == std::vector<std::string>{"revoked"});
}

TEST_CASE("RevokeByShortId only removes the targeted client when two share a "
          "display name",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store =
        TrustStore::Load(persistence, QueuedShortIds({"11111", "22222"}));
    REQUIRE(
        store.Persist("client-1", MakeCredential(1), std::string("Shared Name"))
            .has_value());
    REQUIRE(
        store.Persist("client-2", MakeCredential(2), std::string("Shared Name"))
            .has_value());
    RecordingSessionDisconnector disconnector;
    PairingSession pairingSession;
    FactoryResetChallenge factoryResetChallenge;
    ...
    CHECK(service.RevokeByShortId("11111") ==
          "Revoked client 11111 (Shared Name).");
...
TEST_CASE("RevokeByShortId surfaces a trust-store save failure and leaves the "
          "client trusted",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE("StartFactoryReset returns a message containing the generated code "
          "and mutates nothing",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    ...
    CHECK(service.StartFactoryReset() ==
          "Factory Reset requested. Confirm with code 654321 within 60 seconds "
...
TEST_CASE("StartFactoryReset reports a generator failure",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    ...
    CHECK(
        service.StartFactoryReset() ==
        "Failed to start Factory Reset: could not generate a confirmation code.");
}

TEST_CASE("ConfirmFactoryReset with the correct code wipes trust, cancels "
          "pairing, and disconnects",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store =
        TrustStore::Load(persistence, QueuedShortIds({"11111", "22222"}));
    ...
    REQUIRE(
        pairingSession
            .TryStartChallenge("other-client", std::chrono::steady_clock::now())
            .outcome == dovahlink::security::StartChallengeOutcome::kStarted);
    FactoryResetChallenge factoryResetChallenge(FixedFactoryResetCode("654321"));
    TrustAdminService service(store, disconnector, pairingSession,
                              factoryResetChallenge);
...
TEST_CASE(
    "ConfirmFactoryReset with the wrong code performs no destructive change",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
...
TEST_CASE("ConfirmFactoryReset with no challenge started performs no "
          "destructive change",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    ...
    CHECK(service.ConfirmFactoryReset("000000") ==
          "No Factory Reset confirmation is pending; start one with 'reset' "
          "first.");
...
TEST_CASE("ConfirmFactoryReset surfaces a trust-store save failure, leaves "
          "clients trusted, and "
          "cancels no pairing",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    ...
    CHECK(store.ListTrusted().size() == 1);
    CHECK(disconnector.disconnectActiveCallCount == 0);
    //  The still-owned challenge proves CancelAll was never reached after the save
    //  failure.
    CHECK(pairingSession
              .TryStartChallenge("other-client", std::chrono::steady_clock::now())
              .outcome == dovahlink::security::StartChallengeOutcome::kResumed);
}

TEST_CASE("ConfirmFactoryReset called again after a successful confirm reports "
    ...
    TrustAdminService service(store, disconnector, pairingSession,
                              factoryResetChallenge);
    REQUIRE(service.StartFactoryReset().find("654321") != std::string::npos);
    REQUIRE(service.ConfirmFactoryReset("654321") ==
...
TEST_CASE("StartFactoryReset called twice invalidates the first code",
    ...
    PairingSession pairingSession;
    FactoryResetChallenge factoryResetChallenge(
        QueuedFactoryResetCodes({"111111", "222222"}));
    TrustAdminService service(store, disconnector, pairingSession,
                              factoryResetChallenge);
...
TEST_CASE("ConfirmFactoryReset reports zero trusted devices erased and "
          "singular phrasing for one",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE("ResetTrust revokes every trusted device, cancels pairing, and "
          "disconnects each by clientId",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store =
        TrustStore::Load(persistence, QueuedShortIds({"11111", "22222"}));
    ...
    //  Never the unconditional disconnect -- an unrelated active session (e.g.
    //  developer-token) must survive Reset Trust untouched.
    CHECK(disconnector.disconnectActiveCallCount == 0);
    CHECK(disconnector.disconnectReasons ==
...
TEST_CASE("ResetTrust reports zero devices revoked on an empty store and "
          "disconnects nothing",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
...
TEST_CASE("ResetTrust leaves an unrelated active session (e.g. "
          "developer-token) untouched",
          "[application][trust_admin_service]") {
    //  RecordingSessionDisconnector::DisconnectIfClientActive only ever fires for
    //  the clientIds this call passes it -- a developer-token session (which never
    //  appears in TrustStore, per security.md's "Developer authentication") would
    //  never be one of them, and this test proves ResetTrust never falls back to
    //  the unconditional DisconnectActive that would catch it anyway.
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
        store.Persist("client-1", MakeCredential(1), std::nullopt).has_value());
    ...
    CHECK(disconnector.disconnectedClientIds ==
          std::vector<std::string>{"client-1"});
    CHECK(disconnector.disconnectReasons ==
          std::vector<std::string>{"trust_reset"});
    CHECK(disconnector.disconnectActiveCallCount == 0);
}

TEST_CASE("ResetTrust does not disconnect a Known Device that was already "
          "revoked or blocked",
          "[application][trust_admin_service]") {
    //  Only a device that ResetTrust itself just moved out of kTrusted may be
    //  disconnected -- a device that was already non-trusted before the call owns
    //  no session ResetTrust is responsible for tearing down.
    FakePersistence persistence;
    persistence.SetSnapshotToLoad(
        TrustStoreSnapshot{.devices = {
                               MakeKnownDevice("client-1", "11111", std::nullopt,
                                               KnownDeviceState::kTrusted, 100),
                               MakeKnownDevice("client-2", "22222", std::nullopt,
                                               KnownDeviceState::kRevoked, 200),
                               MakeKnownDevice("client-3", "33333", std::nullopt,
                                               KnownDeviceState::kBlocked, 300),
                           }});
...
TEST_CASE("ResetTrust surfaces a trust-store save failure, leaves clients "
          "trusted, and cancels no "
          "pairing",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE("BlockByShortId reports not-found on an empty store",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
    ...
    CHECK(service.BlockByShortId("11111", now) ==
          "No known device with id 11111.");
    CHECK(disconnector.disconnectedClientIds.empty());
}

TEST_CASE("BlockByShortId blocks a trusted client, disconnects its session, "
          "cancels its owned "
    ...
    FactoryResetChallenge factoryResetChallenge;
    auto now = std::chrono::steady_clock::now();
    REQUIRE(pairingSession.TryStartChallenge("client-1", now).outcome ==
            dovahlink::security::StartChallengeOutcome::kStarted);
    TrustAdminService service(store, disconnector, pairingSession,
                              factoryResetChallenge);

    CHECK(service.BlockByShortId("11111", now) ==
          "Blocked device 11111 (Phone).");
    ...
    //  The owned challenge was cancelled: a fresh TryStartChallenge for the same
    //  clientId starts a brand new one instead of resuming a stale kResumed.
    CHECK(pairingSession.TryStartChallenge("client-1", now).outcome ==
          dovahlink::security::StartChallengeOutcome::kStarted);
}

TEST_CASE("BlockByShortId blocks a revoked client by shortId even though it is "
...
TEST_CASE("BlockByShortId reports already-blocked without disconnecting or "
          "cancelling again",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE("BlockByShortId reports not-eligible for an unpaired client and "
          "cancels no pairing challenge",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
        store.Persist("client-1", MakeCredential(1), std::nullopt).has_value());
    REQUIRE(store.Block("client-1") == BlockOutcome::kBlocked);
    REQUIRE(store.Unblock("client-1") ==
    ...
    CHECK(service.BlockByShortId("11111", now) ==
          "Device 11111 cannot be blocked (not currently trusted or revoked).");

    //  A non-kBlocked outcome must not have touched the owned challenge: it is
    //  still resumable, not cancelled.
    CHECK(pairingSession.TryStartChallenge("client-1", now).outcome ==
          dovahlink::security::StartChallengeOutcome::kResumed);
}

TEST_CASE("BlockByShortId surfaces a trust-store save failure and leaves the "
          "client trusted",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE("UnblockByShortId reports not-found on an empty store",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE(
    "UnblockByShortId unblocks a blocked client and reports its display name",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(store.Persist("client-1", MakeCredential(1), std::string("Phone"))
    ...
    CHECK_FALSE(store.IsBlocked("client-1"));
    //  Unblock never force-closes a session -- a blocked device has none to
    //  disconnect.
    CHECK(disconnector.disconnectedClientIds.empty());
    CHECK(disconnector.disconnectActiveCallCount == 0);
}

TEST_CASE("UnblockByShortId reports not-blocked for a trusted client",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
...
TEST_CASE("UnblockByShortId surfaces a trust-store save failure and leaves the "
          "client blocked",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    ...
    CHECK(service.UnblockByShortId("11111") ==
          "Failed to unblock device 11111: trust-store save failed.");

    CHECK(store.IsBlocked("client-1"));
}

TEST_CASE("RevokeByShortId reports not-found for a blocked device, mirroring "
          "an unknown shortId",
          "[application][trust_admin_service]") {
    //  RevokeByShortId only ever finds a currently-kTrusted device (via
    //  TrustStore::ListTrusted); a blocked device is invisible to it, exactly like
    //  a shortId that was never allocated at all.
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
        store.Persist("client-1", MakeCredential(1), std::nullopt).has_value());
...
TEST_CASE("ForgetByShortId reports not-found on an empty store",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({}));
    RecordingSessionDisconnector disconnector;
...
TEST_CASE("ForgetByShortId reports not-eligible for a trusted client and "
    ...
    CHECK(service.ForgetByShortId("11111") ==
          "Device 11111 cannot be forgotten (revoke or unblock it first).");

    CHECK(store.Query("client-1").has_value());
}

TEST_CASE("ForgetByShortId reports not-eligible for a blocked client without "
          "lifting the block",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
...
TEST_CASE(
    "ForgetByShortId forgets a revoked client and reports its display name",
    "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(store.Persist("client-1", MakeCredential(1), std::string("Phone"))
...
TEST_CASE("ForgetByShortId forgets an unpaired (unblocked) client",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
...
TEST_CASE("ForgetByShortId surfaces a trust-store save failure and leaves the "
          "record intact",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    ...
    persistence.FailNextSave();
    CHECK(service.ForgetByShortId("11111") ==
          "Failed to forget device 11111: trust-store save failed.");

    CHECK(store.FindByShortId("11111").has_value());
}

TEST_CASE("ForgetByShortId only erases the targeted client when two share a "
    ...
    CHECK(service.ForgetByShortId("11111") ==
          "Forgot device 11111 (Shared Name).");
...
TEST_CASE("ForgetByShortId reports not-found when re-forgetting an "
          "already-forgotten shortId",
          "[application][trust_admin_service]") {
    FakePersistence persistence;
    auto store = TrustStore::Load(persistence, QueuedShortIds({"11111"}));
    REQUIRE(
    ...
    TrustAdminService service(store, disconnector, pairingSession,
                              factoryResetChallenge);
    REQUIRE(service.ForgetByShortId("11111") ==
            "Forgot device 11111 ((no display name)).");

...
GameLifecycleTracker::Transition GameLifecycleTracker::Activate() {
    ...
    return Transition{.contextInvalidated = hadActiveContext, .newPlayContextId = std::move(id)};
}

GameLifecycleTracker::Transition GameLifecycleTracker::HandleEvent(LifecycleEvent event) {
    switch (event) {
...
std::optional<std::string> GameLifecycleTracker::CurrentPlayContextId() const { return currentPlayContextId_; }

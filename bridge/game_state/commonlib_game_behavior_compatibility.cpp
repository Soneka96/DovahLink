...
void ApplyAlwaysActiveSetting() {
    ...
    if (!iniSettings) {
        SKSE::log::warn("Could not resolve the INI setting collection; always-active mode was not applied.");
        return;
    }
    auto* setting = iniSettings->GetSetting("bAlwaysActive:General");
    if (!setting) {
        SKSE::log::warn("bAlwaysActive:General was not found; always-active mode was not applied.");
        return;
    }
    setting->data.b = true;
    SKSE::log::info("Always-active mode applied (bAlwaysActive:General set to true).");
}

void InstallAchievementCompatibilityPatch() {
    REL::Relocation<std::uintptr_t> target{REL::RelocationID(kAchievementFunctionSeId, kAchievementFunctionAeId)};
    if (target.address() == 0) {
        SKSE::log::warn(
            "Could not resolve the achievement-eligibility function address; achievement compatibility was not "
            "applied.");
        return;
    }
    ...
    patch.ready();
    REL::safe_write(target.address(), patch.getCode<const void*>(), patch.getSize());

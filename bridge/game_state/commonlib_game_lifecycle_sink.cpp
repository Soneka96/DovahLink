...
void CommonLibGameLifecycleSink::OnMessage(const SKSE::MessagingInterface::Message& message) {
    ...
    try {
        ...
        if (callbackRunner_) {
            (void)callbackRunner_(
                [this, event, description = std::move(rawDescription)] { HandleEvent(event, description); });
        }
    } catch (...) {
...
void CommonLibGameLifecycleSink::OnRevert() {
    try {
        if (callbackRunner_) {
            (void)callbackRunner_([this] { HandleEvent(application::LifecycleEvent::kRevert, "Revert"); });
        }
    } catch (...) {
...
void CommonLibGameLifecycleSink::HandleEvent(application::LifecycleEvent event, std::string_view rawDescription) {

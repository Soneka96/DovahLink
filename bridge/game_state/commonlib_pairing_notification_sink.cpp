...
void CommonLibPairingNotificationSink::NotifyPairingCodeIncorrect(std::string_view sixDigitCode) {
    std::string message = "DovahLink: wrong code. Your pairing code is: " + std::string(sixDigitCode);
    RE::DebugNotification(message.c_str());
}

void CommonLibPairingNotificationSink::NotifyPairingAttemptsExhausted() {
    RE::DebugNotification("DovahLink: too many wrong attempts. Request pairing again.");
}

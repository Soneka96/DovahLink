...
TEST_CASE("CaptureLevel returns the raw value for a positive level", "[game_state][level_adapter]") {
...
TEST_CASE("CaptureLevel treats a raw level of zero as untrustworthy, not a real value",
          "[game_state][level_adapter]") {
    FakeLevelAccessor accessor(0);
    CHECK_FALSE(CaptureLevel(accessor).has_value());
}

TEST_CASE("CaptureLevel treats a negative raw level as untrustworthy", "[game_state][level_adapter]") {
    FakeLevelAccessor accessor(-1);
    CHECK_FALSE(CaptureLevel(accessor).has_value());
}

TEST_CASE("CaptureLevel returns level 1, the minimum real Skyrim level, unchanged",
          "[game_state][level_adapter]") {
    FakeLevelAccessor accessor(1);
    auto level = CaptureLevel(accessor);
    REQUIRE(level.has_value());
...
TEST_CASE("CaptureLevel returns a high level unchanged", "[game_state][level_adapter]") {

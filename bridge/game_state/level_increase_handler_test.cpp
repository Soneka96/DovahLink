...
TEST_CASE("HandleLevelIncrease pushes nullopt when the accessor has no trustworthy level",
    ...
    Verify(Method(accessor, ReadLevel)).Once();
    Verify(Method(sink, OnLevelCaptured).Using(std::optional<std::int64_t>{})).Once();
    VerifyNoOtherInvocations(accessor, sink);
}
...
TEST_CASE("HandleLevelIncrease re-reads the accessor on every call rather than caching",
    ...
    When(Method(accessor, ReadLevel))
        .Return(std::optional<std::int64_t>{10}, std::optional<std::int64_t>{11}, std::nullopt);
    When(Method(sink, OnLevelCaptured))
        .AlwaysDo([](const std::optional<std::int64_t>&) {});
    LevelIncreaseHandler handler(accessor.get(), sink.get());
    ...
    Verify(Method(sink, OnLevelCaptured).Using(std::optional<std::int64_t>{10})).Once();
    Verify(Method(sink, OnLevelCaptured).Using(std::optional<std::int64_t>{11})).Once();
    Verify(Method(sink, OnLevelCaptured).Using(std::optional<std::int64_t>{})).Once();
    VerifyNoOtherInvocations(accessor, sink);
}

...
struct HelloAckPayload {
  ...
  /// Kind of client identity established by this handshake: `"unpaired"` for a
  /// session admitted via `one_time_local_token` or the bootstrap `unpaired`
  /// auth method (trust-restricted until pairing succeeds), or `"paired"` for a
  /// session admitted via `trusted_device_credential`, or a restricted session
  /// upgraded in place by a successful pairing confirmation. See

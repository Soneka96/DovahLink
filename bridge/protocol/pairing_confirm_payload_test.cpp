...
TEST_CASE("pairing_confirm decodes a null displayName as absent", "[protocol][pairing_confirm_payload]") {
...
TEST_CASE("pairing_confirm decodes a missing displayName as absent", "[protocol][pairing_confirm_payload]") {
...
TEST_CASE("pairing_confirm is rejected when code is missing", "[protocol][pairing_confirm_payload]") {
    boost::json::object payload = boost::json::parse(R"({"displayName": "My PC"})").get_object();
    auto confirm = dovahlink::protocol::DecodePairingConfirmPayload(payload);
    REQUIRE_FALSE(confirm.has_value());
}

TEST_CASE("pairing_confirm is rejected when code is an empty string", "[protocol][pairing_confirm_payload]") {
    boost::json::object payload = boost::json::parse(R"({"code": ""})").get_object();
    auto confirm = dovahlink::protocol::DecodePairingConfirmPayload(payload);
    REQUIRE_FALSE(confirm.has_value());
}

TEST_CASE("pairing_confirm is rejected when code has the wrong type", "[protocol][pairing_confirm_payload]") {
    boost::json::object payload = boost::json::parse(R"({"code": 123456})").get_object();
    auto confirm = dovahlink::protocol::DecodePairingConfirmPayload(payload);
    REQUIRE_FALSE(confirm.has_value());
}

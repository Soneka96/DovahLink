...
struct PairingStatusPayload {
  ...
  std::string state;
  /// The active challenge's remaining code validity in seconds; present for
  /// `"available"` (a freshly started challenge) and `"in_progress"` (the
  /// caller's own challenge, resumed). Never present for `"unavailable"` or
  /// `"other_device_pairing"` -- the latter reveals nothing about the owning
  /// device's code or its remaining time.

...
TEST_CASE("pairing-ack fixture decodes to the expected PairingAckPayload", "[protocol][pairing_ack_payload]") {
...
TEST_CASE("pairing_ack is rejected when credential is missing", "[protocol][pairing_ack_payload]") {
...
TEST_CASE("pairing_ack is rejected when credential is an empty string", "[protocol][pairing_ack_payload]") {
    boost::json::object payload = boost::json::parse(R"({"credential": ""})").get_object();
    auto ack = dovahlink::protocol::DecodePairingAckPayload(payload);
    REQUIRE_FALSE(ack.has_value());
}

TEST_CASE("pairing_ack is rejected when credential has the wrong type", "[protocol][pairing_ack_payload]") {

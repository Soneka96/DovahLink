...
/// Builds a complete error envelope and always returns a usable envelope. Uses
/// the `csprng-unavailable` sentinel message ID if secure ID generation fails.
/// `correlationId` identifies the message being answered or is null when there
/// is no correlation; `sessionId` is null before a session exists and is

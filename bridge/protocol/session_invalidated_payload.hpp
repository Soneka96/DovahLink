...
/// Encodes a session-invalidated payload. Server-originated only; the client never sends this
/// message back, so no matching `Decode...` function exists.
boost::json::object EncodeSessionInvalidatedPayload(const SessionInvalidatedPayload& payload);

/// Builds a complete, unsolicited `session_invalidated` envelope and always returns a usable
/// envelope. Uses the `csprng-unavailable` sentinel message ID if secure ID generation fails,
/// matching `BuildErrorEnvelope`. `correlationId` is always null -- this event answers nothing.

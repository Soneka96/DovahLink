...
TEST_CASE("error-rate-limited fixture decodes as retryable", "[protocol][error_payload]") {
...
TEST_CASE("every error fixture decodes to a valid ErrorPayload", "[protocol][error_payload]") {
...
TEST_CASE("error is rejected when retryable is missing", "[protocol][error_payload]") {
...
TEST_CASE("error accepts non-object, non-null details", "[protocol][error_payload]") {
...
TEST_CASE("error decodes non-null details when present", "[protocol][error_payload]") {
    boost::json::object payload =
        boost::json::parse(
...
TEST_CASE("EncodeErrorPayload round-trips a fixture with details absent", "[protocol][error_payload]") {
...
TEST_CASE("EncodeErrorPayload round-trips retryable and non-retryable values", "[protocol][error_payload]") {
    auto envelope = DecodeFixtureEnvelope("errors/error-rate-limited.json");
    auto original = dovahlink::protocol::DecodeErrorPayload(envelope.payload);
    REQUIRE(original.has_value());
...
TEST_CASE("EncodeErrorPayload round-trips a payload with details actually present", "[protocol][error_payload]") {
...
TEST_CASE("BuildErrorEnvelope carries the caller's sessionId through unchanged", "[protocol][error_payload]") {

...
TEST_CASE("pairing_status decodes a missing expiresInSeconds as absent", "[protocol][pairing_status_payload]") {
...
TEST_CASE("pairing_status accepts the other_device_pairing state", "[protocol][pairing_status_payload]") {
...
TEST_CASE("pairing_status is rejected when expiresInSeconds is negative", "[protocol][pairing_status_payload]") {
...
TEST_CASE("PairingStatusPayload round-trips through encode then decode", "[protocol][pairing_status_payload]") {
...
TEST_CASE("pairing_status is rejected when state is not a registered value", "[protocol][pairing_status_payload]") {
    boost::json::object payload = boost::json::parse(R"({"state": "sideways"})").get_object();
    auto status = dovahlink::protocol::DecodePairingStatusPayload(payload);
    REQUIRE_FALSE(status.has_value());
}

TEST_CASE("pairing_status is rejected when state has the wrong type", "[protocol][pairing_status_payload]") {

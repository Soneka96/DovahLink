...
TEST_CASE(
    "snapshot-request fixture decodes to the expected SnapshotRequestPayload",
    "[protocol][snapshot_request_payload]") {
    auto envelope = DecodeFixtureEnvelope("subscriptions/snapshot-request.json");
    auto snapshotRequest =
        dovahlink::protocol::DecodeSnapshotRequestPayload(envelope.payload);
...
TEST_CASE("snapshot-request decodes without knownRevision when it is absent",
    ...
    boost::json::object payload =
        boost::json::parse(R"({"stateArea": "example_area"})").get_object();
    auto snapshotRequest =
        dovahlink::protocol::DecodeSnapshotRequestPayload(payload);
    REQUIRE(snapshotRequest.has_value());

...
TEST_CASE("hello fixture decodes to the expected HelloPayload",
...
TEST_CASE("hello is rejected when clientId is null",
          "[protocol][hello_payload]") {
...
TEST_CASE("hello is rejected when clientId is missing",
          "[protocol][hello_payload]") {
    boost::json::object payload = boost::json::parse(
                                      R"({"endpoint": "client",
            "auth": {"method": "one_time_local_token", "token": "t"}})")
...
TEST_CASE("hello-unpaired fixture decodes with no authToken",
          "[protocol][hello_payload]") {
...
TEST_CASE("hello-trusted-device-credential fixture decodes with an authToken",
          "[protocol][hello_payload]") {
    auto envelope =
        DecodeFixtureEnvelope("connection/hello-trusted-device-credential.json");
    auto hello = dovahlink::protocol::DecodeHelloPayload(envelope.payload);
    REQUIRE(hello.has_value());
...
TEST_CASE("hello is rejected when auth.method is unrecognized",
          "[protocol][hello_payload]") {
...
TEST_CASE(
    "hello is rejected when auth.method is unpaired but a token is present",
    "[protocol][hello_payload]") {
    boost::json::object payload =
        boost::json::parse(
            R"({"endpoint": "client", "clientId": "client-1",
...
TEST_CASE("hello is rejected when auth.method is unpaired but token is present "
          "as JSON null",
          "[protocol][hello_payload]") {
    //  A present-but-null key is a distinct wire case from an absent key:
    //  if_contains finds it either way, so the rejection must not depend on the
    //  key being entirely missing.
    boost::json::object payload =
        boost::json::parse(
            R"({"endpoint": "client", "clientId": "client-1",
...
TEST_CASE("hello is rejected when auth.method is trusted_device_credential but "
          "no token is present",
          "[protocol][hello_payload]") {
    boost::json::object payload =
        boost::json::parse(
...
TEST_CASE("hello is rejected when endpoint is not 'client'",
          "[protocol][hello_payload]") {
    boost::json::object payload =
        boost::json::parse(
...
TEST_CASE("hello is rejected when auth.method is not 'one_time_local_token'",
          "[protocol][hello_payload]") {
    boost::json::object payload =
        boost::json::parse(
            R"({"endpoint": "client", "clientId": "client-1",
...
TEST_CASE("hello is rejected when auth is missing",
          "[protocol][hello_payload]") {
    boost::json::object payload =
        boost::json::parse(R"({"endpoint": "client", "clientId": "client-1"})")
            .get_object();
    auto hello = dovahlink::protocol::DecodeHelloPayload(payload);
    REQUIRE_FALSE(hello.has_value());
}

TEST_CASE("hello is rejected when clientId is present but not a string",
          "[protocol][hello_payload]") {
    boost::json::object payload = boost::json::parse(
...
TEST_CASE("hello is rejected when clientId is an empty string",
          "[protocol][hello_payload]") {
    boost::json::object payload = boost::json::parse(

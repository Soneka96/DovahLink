...
public:
    ...
    [[nodiscard]] boost::asio::ip::tcp::endpoint LocalEndpoint() const { return endpoint_; }
    ...
    /// Returns whether the server thread has posted its pending accept within the supplied duration.
    template <class Rep, class Period>
    [[nodiscard]] bool WaitForAcceptReady(const std::chrono::duration<Rep, Period>& timeout) {
        std::unique_lock lock(acceptReadyMutex_);
        return acceptReadyChanged_.wait_for(lock, timeout, [this] { return acceptReady_; });
    }
...
private:
    ...
    void Run(std::stop_token stopToken, ContextWork work) noexcept {
        try {
            auto acceptedSocket = std::make_shared<boost::asio::ip::tcp::socket>(ioc_);
            if (stopToken.stop_requested() || cancellationRequested_.load(std::memory_order_acquire)) {
                acceptError_ = boost::asio::error::operation_aborted;
            } else {
                auto asyncAcceptError = std::make_shared<boost::system::error_code>();
                ioc_.restart();
                listener_.Acceptor().async_accept(*acceptedSocket, [asyncAcceptError](boost::system::error_code error) {
    ...
    /// Connects to the listener to wake a pending accept when posting cancellation fails.
    ...
    /// Cached before the server thread starts so callers never read the acceptor concurrently.

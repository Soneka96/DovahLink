...
TEST_CASE("Create binds an IPv4 listener to exactly 127.0.0.1", "[transport][listener]") {
...
TEST_CASE("Create binds an IPv6 listener to exactly ::1", "[transport][listener]") {
    boost::asio::io_context ioc;
...
TEST_CASE("port 0 lets the OS assign an ephemeral port for each listener", "[transport][listener]") {
    boost::asio::io_context ioc;
    auto first = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, 0);
    auto second = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, 0);
...
TEST_CASE("binding a second IPv4 listener to a port already in use fails", "[transport][listener]") {
    boost::asio::io_context ioc;
    auto first = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, 0);
    REQUIRE(first.has_value());
    std::uint16_t boundPort = first->LocalEndpoint().port();

    auto second = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, boundPort);
    REQUIRE_FALSE(second.has_value());
    CHECK(second.error() == ListenerError::kBindFailed);
}

TEST_CASE("binding a second IPv6 listener to a port already in use fails", "[transport][listener]") {
    boost::asio::io_context ioc;
    auto first = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV6, 0);
    REQUIRE(first.has_value());
    std::uint16_t boundPort = first->LocalEndpoint().port();

    auto second = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV6, boundPort);
    REQUIRE_FALSE(second.has_value());
    CHECK(second.error() == ListenerError::kBindFailed);
}

TEST_CASE("a moved-from listener's new owner still accepts connections", "[transport][listener]") {
    boost::asio::io_context ioc;
    auto created = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, 0);
...
TEST_CASE("an IPv4 and an IPv6 listener can coexist on the same io_context", "[transport][listener]") {
    boost::asio::io_context ioc;
...
TEST_CASE("a real client can connect to the IPv4 listener and the server can accept it",
          "[transport][listener]") {
    boost::asio::io_context ioc;
    auto listener = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV4, 0);
    REQUIRE(listener.has_value());
...
TEST_CASE("a real client can connect to the IPv6 listener and the server can accept it",
          "[transport][listener]") {
    boost::asio::io_context ioc;
    auto listener = LoopbackListener::Create(ioc, LoopbackListener::IpVersion::kV6, 0);
    REQUIRE(listener.has_value());

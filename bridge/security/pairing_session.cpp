...
void PairingSession::ExpirePendingIfElapsedLocked(std::chrono::steady_clock::time_point now) {
...
StartChallengeResult PairingSession::TryStartChallenge(const std::string& clientId,
    ...
    // Clears any stale owner/attempt/cooldown bookkeeping left behind by a challenge that expired
    // naturally (via TokenStore's own TTL, which the branches above don't otherwise clean up).
    ClearChallengeLocked();
...
ConfirmCodeResult PairingSession::TryConfirmCode(const std::string& presentedCode,
    ...
    // its real owner, and never even reaches the expiry/pacing/evaluation logic below.
    if (ownerClientId_.has_value() && *ownerClientId_ != clientId) {
        return {.outcome = ConfirmResult::kInvalid, .shouldAutoRenotify = false};
    }
    // Checked ahead of pacing/TryReserve (which would themselves fail identically either way) so a
    // caller can be told "expired" instead of the less useful "invalid" or "pacing_limited".
    ...
    if (!reservation.has_value()) {
        ...
        if (wrongAttempts_ >= kPairingMaxWrongAttempts) {
            ClearChallengeLocked();
            return {.outcome = ConfirmResult::kHardLimitReached, .shouldAutoRenotify = false};
        }
        bool shouldAutoRenotify =
            !autoRenotifyCooldownUntil_.has_value() || now >= *autoRenotifyCooldownUntil_;
        if (shouldAutoRenotify) {
            autoRenotifyCooldownUntil_ = now + kPairingRenotifyCooldown;
        }
        return {.outcome = ConfirmResult::kInvalid, .shouldAutoRenotify = shouldAutoRenotify};
    }
...
RenotifyResult PairingSession::TryRenotify(const std::string& clientId,
    ...
    return {.outcome = RenotifyOutcome::kRenotified, .retryAfterSeconds = std::nullopt};
}

CancelOutcome PairingSession::TryCancel(const std::string& clientId, std::chrono::steady_clock::time_point now) {
    std::lock_guard<std::mutex> lock(mutex_);
    ExpireOwnerIfGraceElapsedLocked(now);
    ExpirePendingIfElapsedLocked(now);
    ExpireChallengeIfElapsedLocked();

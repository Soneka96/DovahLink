...
TEST_CASE("ConstantTimeEquals reports true for identical non-empty buffers", "[security][constant_time_compare]") {
    std::vector<std::uint8_t> a{1, 2, 3, 4};
    std::vector<std::uint8_t> b{1, 2, 3, 4};
    CHECK(ConstantTimeEquals(a, b));
}

TEST_CASE("ConstantTimeEquals reports true for two empty buffers", "[security][constant_time_compare]") {
    CHECK(ConstantTimeEquals({}, {}));
}

TEST_CASE("ConstantTimeEquals reports false for equal-length buffers differing in one byte",
...
TEST_CASE("ConstantTimeEquals reports false for differing lengths regardless of shared prefix",
          "[security][constant_time_compare]") {
...
TEST_CASE("ConstantTimeEquals reports false for an empty buffer against a non-empty one",
          "[security][constant_time_compare]") {
...
TEST_CASE("ConstantTimeEquals reports false the same way whether buffers differ at the first byte, "

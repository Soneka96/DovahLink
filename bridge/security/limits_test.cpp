...
TEST_CASE("input limits match the documented Phase 1 values", "[security][limits]") {
...
TEST_CASE("rate and session limits match the documented Phase 1 values", "[security][limits]") {
    CHECK(kMaxInboundMessagesPerSecond == 100);
    CHECK(kMaxMessagesPerSession == 10'000);
    CHECK(kMaxConnectedClients == 1);
}

TEST_CASE("timeouts match the documented Phase 1 values", "[security][limits]") {
    CHECK(kHandshakeTimeout == std::chrono::seconds{5});
    CHECK(kIdleTimeout == std::chrono::seconds{60});
}

TEST_CASE("outbound queue lanes match the documented Phase 1 values", "[security][limits]") {
    CHECK(kReservedControlRecoverySlots == 16);
    CHECK(kReservedEventSlots == 112);
    CHECK(kMaxOutboundQueueMessages == 128);
    CHECK(kMaxOutboundQueueMessages == kReservedControlRecoverySlots + kReservedEventSlots);
}

TEST_CASE("throttling limits match the documented Phase 1 values", "[security][limits]") {

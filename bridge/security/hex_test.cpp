...
TEST_CASE("EncodeHex produces lowercase two-digit-per-byte output",
          "[security][hex]") {
    std::vector<std::uint8_t> bytes{0x00, 0x0F, 0xAB, 0xFF};
    CHECK(EncodeHex(bytes) == "000fabff");
}

TEST_CASE("EncodeHex of an empty vector is an empty string",
          "[security][hex]") {
    CHECK(EncodeHex({}).empty());
}

TEST_CASE("DecodeHex decodes lowercase and uppercase hex to the same bytes",
          "[security][hex]") {
...
TEST_CASE("EncodeHex then DecodeHex round-trips arbitrary bytes",
          "[security][hex]") {
    std::vector<std::uint8_t> bytes{0x01, 0x23, 0x45, 0x67,
                                    0x89, 0xAB, 0xCD, 0xEF};

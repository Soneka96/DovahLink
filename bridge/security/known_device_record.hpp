...
/// One persistent Known Device's identity, administration metadata, and current trust state.
/// Always in exactly one of four durable states (`KnownDeviceState`); only `kTrusted` carries a
/// usable `credential`.
struct KnownDeviceRecord {
    /// The client's stable protocol identity.
    std::string clientId;
    /// The strong device-scoped credential bound to `clientId`. Empty except while `kTrusted`.
    std::vector<std::uint8_t> credential;
    /// Five-digit administration-only identifier, unique among every currently known device
    /// regardless of state. Never authentication or authorization material. Allocated once, at
    /// first successful pairing, and stable for the record's entire lifetime.
    std::string shortId;

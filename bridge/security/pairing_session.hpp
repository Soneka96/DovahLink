...
class PairingSession final : public IPairingSession {
  public:
    ...
    ///  @param codeGenerator Produces each six-digit code `TryStartChallenge`
    ///  issues; defaults to
    ///      the real CSPRNG-backed generator. Tests inject a deterministic
    ///      generator here.
    ///  @param codeTimeToLive How long a started challenge's code remains valid.
    ///  Tests use a short
    ///      or zero duration to exercise expiry deterministically, matching
    ///      `TokenStore`'s own constructor.
    ...
    ///  Starts a new challenge (`NONE -> CHALLENGE_ACTIVE`) if none is active and
    ///  no credential is currently pending finalization, or reports the existing
    ///  challenge/pending credential's ownership per
    ///  `ai/context/protocol/security.md`'s Phase 3.1 ownership model. Applies the
    ...
    ///  @return `kStarted` with the six-digit code to display; `kResumed` when
    ///  `clientId` already
    ///      owns the active challenge or pending credential; `kOtherDeviceActive`
    ///      when a different `clientId` owns it; or `kGeneratorFailed` when the
    ///      underlying random source fails.
    ...
    ///  check.
    ///  @return The remaining duration, or `std::nullopt` when no challenge is
    ///  active (including a
    ...
    ///  pairing handler can redisplay the same code through the notification sink
    ///  -- for a wrong-attempt auto-renotify or a manual `TryRenotify` -- without
    ///  `PairingSession` itself touching Skyrim or the wire; this is not a
    ///  wire-exposed value, and the code is never sent to the client over the
    ///  connection.
    ///  @param now Current monotonic time, for the reconnect-grace lazy-expiry
    ///  check.
    ...
    ///  active challenge (`CHALLENGE_ACTIVE`). A no-op for a non-owner, or once
    ///  the challenge has reached `PENDING_CREDENTIAL` -- the grace period does
    ///  not apply there; a pending credential's own 5-minute expiry governs
    ///  instead.
    ...
    ///  first, so a reconnect arriving after the grace period already elapsed
    ///  correctly finds no ownership left to resume.
    ...
    ///  rejects as `kInvalid` -- indistinguishable from a wrong code -- when a
    ///  different `clientId` owns the active challenge, so a non-owner cannot
    ///  blind-guess a code that is not theirs. A pacing check
    ///  (`kPairingConfirmPacingInterval`) rejects an attempt evaluated too soon
    ///  after the previous one as `kPacingLimited` without counting it as wrong; a
    ///  wrong evaluated attempt counts toward `kPairingMaxWrongAttempts`, and the
    ...
    ///  pacing/attempt
    ///      accounting.
    ///  @param clientId The pairing client's identity, checked against the
    ///  challenge owner and
    ///      held pending.
    ...
    ///  not match, is
    ///      expired, or none is pending (including after a Bridge restart lost
    ///      it).
    ...
    ///  notification sink, never a new one. Never reveals or returns the code
    ///  itself; the caller already has it from the original display. Independent
    ///  of the auto-renotify cooldown `TryConfirmCode` manages for wrong-code
    ///  redisplays -- neither affects the other.
    ///  @param clientId The requesting client's identity.
    ///  @param now Current monotonic time, for the reconnect-grace lazy-expiry
    ///  check and cooldown
    ///     accounting.
    ///  @return `kRenotified` when the caller should redisplay the code now;
    ///  `kCooldown` with the
    ///     remaining wait when it owns the challenge but must wait; `kNotActive`
    ...
    ///  reports `kAlreadyIdle`, not `kCancelled`, when `clientId` owns nothing.
    ///  Never touches `TrustStore` or any already- committed trust; only ever
    ///  clears in-memory challenge/pending state.
    ///  @param clientId The requesting client's identity.
    ///  @param now Current monotonic time, for the lazy-expiry checks.
    ...
    ///  ever affects the calling `clientId`'s own ownership, this is for an
    ///  administrative bulk operation (Reset Trust, Factory Reset) that must
  ...
  private:
    ...
    ///  set and `now - *disconnectedAt_ >= kPairingReconnectGracePeriod`, per
    ///  `ai/context/protocol/security.md`'s reconnect-grace lazy-expiry model --
    ///  evaluated on next access rather than by an active timer. A no-op when no
    ///  disconnect is pending. Call while holding `mutex_`.
    ///  @param now Current monotonic time.
    void
    ExpireOwnerIfGraceElapsedLocked(std::chrono::steady_clock::time_point now);

    ///  Clears `pendingCredential_` once `now - pendingCredential_->pendingSince
    ///  >= kPairingPendingCredentialTtl`, evaluated on next access rather than by
    ///  an active timer. A no-op when no credential is pending. Call while holding
    ///  `mutex_`.
    ///  @param now Current monotonic time.
    void ExpirePendingIfElapsedLocked(std::chrono::steady_clock::time_point now);

    ///  Clears `activeChallenge_` once its own `TokenStore::IsAvailable` reports
    ///  the code's TTL has naturally elapsed -- distinct from
    ///  `ExpireOwnerIfGraceElapsedLocked`'s disconnect-driven grace period, this
    ///  fires even while the owner stays continuously connected. Without this,
    ///  `CurrentCode`/`TryRenotify`/`TryCancel` could treat a challenge past its
    ///  own `RemainingSeconds` as still active, since only `TryConfirmCode` (via
    ///  its own explicit `kExpired` outcome) and `TryStartChallenge` (via its own
    ///  explicit `IsAvailable` check) already accounted for this. A no-op when no
    ///  challenge is active or it has not yet expired. Call while holding
    ///  `mutex_`.
    void ExpireChallengeIfElapsedLocked();

    ///  Resets every field scoped to one active challenge's lifetime
    ///  (`activeChallenge_`, `activeCode_`, `ownerClientId_`, `disconnectedAt_`,
    ///  the wrong-attempt/pacing counters, and both renotify cooldowns) back to
    ///  their `NONE` state together, so no field can accidentally carry stale data
    ///  into the next challenge. Call while holding `mutex_`. Never touches
    ...
    ///  The same code `activeChallenge_` validates against, kept in plaintext for
    ///  `CurrentCode`'s redisplay use. Retained for exactly `activeChallenge_`'s
    ///  own lifetime -- no longer-lived than the plaintext this handler already
    ///  returns once from `TryStartChallenge` itself.
    std::optional<std::string> activeCode_;

    ///  The `clientId` that owns `activeChallenge_`, or no value when it does not
    ///  exist (`NONE` or `PENDING_CREDENTIAL` -- cleared together with
    ///  `activeChallenge_` on every exit from `CHALLENGE_ACTIVE`).
    ///  Pending-credential ownership is tracked separately by
    ///  `PendingCredential::clientId`.
    std::optional<std::string> ownerClientId_;

    ///  When `ownerClientId_`'s connection disconnected while owning
    ///  `activeChallenge_`, or no value while it is connected or no challenge is
    ///  owned. Cleared by `NotifyReconnected` or by
    ///  `ExpireOwnerIfGraceElapsedLocked` once the grace period elapses. Never set
    ///  while the challenge is `PENDING_CREDENTIAL` -- the grace period does not
    ///  apply there.
    std::optional<std::chrono::steady_clock::time_point> disconnectedAt_;

    ///  Wrong evaluated `TryConfirmCode` attempts against `activeChallenge_` so
    ///  far, reset whenever a fresh challenge starts. The attempt that reaches
    ///  `kPairingMaxWrongAttempts` cancels the challenge via
    ///  `ClearChallengeLocked`, so this can never itself hold a stale count from a
    ///  prior challenge.
    std::size_t wrongAttempts_ = 0;

    ///  When `TryConfirmCode` last actually evaluated a code against
    ///  `activeChallenge_` (passed the ownership and expiry checks and reached
    ///  `TokenStore::TryReserve`), or no value before the first evaluated attempt.
    ///  Backs the `kPairingConfirmPacingInterval` pacing check -- distinct from
    ///  `wrongAttempts_`, since a paced-out attempt is never evaluated and never
    ///  counts as wrong.
    std::optional<std::chrono::steady_clock::time_point> lastConfirmAttemptAt_;

    ///  When `TryRenotify`'s manual "show code again" cooldown next allows a fresh
    ///  redisplay, or no value before the first renotify. Independent of
    ///  `autoRenotifyCooldownUntil_`.
    std::optional<std::chrono::steady_clock::time_point> renotifyCooldownUntil_;

    ///  When `TryConfirmCode`'s automatic wrong-code redisplay cooldown next
    ///  allows another automatic redisplay, or no value before the first one.
    ///  Independent of `renotifyCooldownUntil_`; reuses the same
    ///  `kPairingRenotifyCooldown` duration as a deliberate choice (both exist to
    ///  prevent Skyrim notification spam for the same code), but is tracked
    ///  separately so neither path's cooldown affects the other's.

...
class FactoryResetChallenge : public IFactoryResetChallenge {
  public:
    ...
    ///  @param codeGenerator Produces each six-digit code `TryStart` issues;
    ...
    ///  @return `kConfirmed` on a match, consuming the challenge; `kExpired` when
    ///  no challenge is
  ...
  private:
    ...
    ///  The active challenge's single-use, expiring code, or no value when none is
    ///  active.

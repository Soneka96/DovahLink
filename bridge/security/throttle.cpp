...
std::size_t RateWindowCounter::ActiveCount(std::chrono::steady_clock::time_point now) {
    std::lock_guard<std::mutex> lock(mutex_);
    PruneLocked(now);
    return eventTimes_.size();
}
...
void FailedTokenThrottle::RecordFailure(std::chrono::steady_clock::time_point now) {

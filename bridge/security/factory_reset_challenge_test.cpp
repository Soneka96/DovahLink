...
/// `FactoryResetChallenge::CodeGenerator` that fails once (simulating a transient CSPRNG
/// failure), then returns `code` on every later call.
...
FactoryResetChallenge::CodeGenerator SequentialCodes(std::deque<std::string> codes) {
...
TEST_CASE("TryStart returns the generated six-digit code", "[security][factory_reset_challenge]") {
...
TEST_CASE("TryStart propagates a code-generator failure", "[security][factory_reset_challenge]") {
    FactoryResetChallenge challenge(FailingCodeGenerator());

    CHECK_FALSE(challenge.TryStart().has_value());
}

TEST_CASE("TryStart succeeds on a later call after an earlier generator failure",
    ...
    CHECK(challenge.TryConfirm("123456") == FactoryResetConfirmOutcome::kConfirmed);
}

TEST_CASE("TryConfirm with the correct code confirms and consumes the challenge",
          "[security][factory_reset_challenge]") {
    FactoryResetChallenge challenge(FixedCode("123456"));
...
TEST_CASE("TryConfirm with a wrong code destroys the challenge immediately",
    ...
    // No multi-attempt budget: the correct code no longer works either, since the one wrong
    // attempt already destroyed the challenge.
    CHECK(challenge.TryConfirm("123456") == FactoryResetConfirmOutcome::kExpired);
}

TEST_CASE("TryConfirm with a wrong-length or empty presented code is treated as a wrong code",
          "[security][factory_reset_challenge]") {
    FactoryResetChallenge challenge(FixedCode("123456"));
    REQUIRE(challenge.TryStart().has_value());

    CHECK(challenge.TryConfirm("1234567") == FactoryResetConfirmOutcome::kInvalid);
}
...
TEST_CASE("TryConfirm reports kExpired once the challenge's own TTL has elapsed",
          "[security][factory_reset_challenge]") {
...
TEST_CASE("TryStart discards any previous challenge unconditionally",
    ...
    // A fresh TryStart always succeeds and generates a new code -- there is no ownership model to
    // resume, unlike PairingSession's kResumed outcome.
    auto second = challenge.TryStart();
    REQUIRE(second.has_value());
    CHECK(*second == "222222");

    // The discarded first code no longer confirms anything against the fresh challenge.

...
class RateWindowCounter {
  public:
    ...
    RecordEvent(std::chrono::steady_clock::time_point now);

    ///  Returns the active event count without recording a new event.
    [[nodiscard]] std::size_t
    ActiveCount(std::chrono::steady_clock::time_point now);
...
class ViolationTracker {
  public:
    ...
    [[nodiscard]] bool
    RecordViolationAndCheckLimit(std::chrono::steady_clock::time_point now);
...
class InboundMessageRateLimiter {
  public:
    ...
    [[nodiscard]] bool
    RecordMessageAndCheckLimit(std::chrono::steady_clock::time_point now);

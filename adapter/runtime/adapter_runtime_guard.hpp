...
struct RuntimeVersion {
  ...
  friend bool operator==(const RuntimeVersion &,
                         const RuntimeVersion &) = default;
};

///  Supported Skyrim runtime version for plugin initialization, matching
///  `bridge/game_state/runtime_guard.hpp`'s `kSupportedSkyrimVersion`.
inline constexpr RuntimeVersion kSupportedSkyrimVersion{1, 6, 1170, 0};
///  Supported SKSE runtime version for plugin initialization, matching
///  `bridge/game_state/runtime_guard.hpp`'s `kSupportedSkseVersion`.
...
#if defined(_WIN32)
  //  VersionHelpers uses the host process manifest. SKSE loads this DLL into
  //  Skyrim, whose manifest is outside the Adapter's control, so use the
  //  kernel's version directly instead.
  auto *ntdll = GetModuleHandleW(L"ntdll.dll");
  if (ntdll == nullptr) {
    return false;
  }
  auto *getVersion = reinterpret_cast<LONG(WINAPI *)(OSVERSIONINFOEXW *)>(
...
#else
  //  The native Adapter target is Windows-only. Keep the neutral test target
  //  usable on non-Windows hosts without pretending to validate Windows
  //  there.
  return true;
#endif
}
...
IsSupportedSkyrimVersion(const RuntimeVersion &version) {
  return version == kSupportedSkyrimVersion;
}

///  Returns whether `version` matches the supported SKSE runtime.
[[nodiscard]] constexpr bool
IsSupportedSkseVersion(const RuntimeVersion &version) {

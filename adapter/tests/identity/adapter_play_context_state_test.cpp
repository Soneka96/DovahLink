...
TEST_CASE("AdapterPlayContextState reports all-zero before any value is set",
          "[identity][adapter_play_context_state]") {
    AdapterPlayContextState state;

    CHECK(state.CurrentPlayContext() == std::array<std::byte, 16>{});
}
    ...
    for (int reader = 0; reader < 4; ++reader) {
        threads.emplace_back([&] {
            for (int iteration = 0; iteration < kWriteCount; ++iteration) {
                std::array<std::byte, 16> observed = state.CurrentPlayContext();

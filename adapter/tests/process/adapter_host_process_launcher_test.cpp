...
TEST_CASE("Win32AdapterHostProcessLauncher inherits only the rendezvous pipe",
    ...
    //  HOSTPROOF ef confirms the child ran the event-signal probe. The parent
    //  event must remain unsignaled: if the child inherited this event object,
    //  its SetEvent call would have changed the same object. A different child
    //  handle may reuse the same numeric value without being a leak.
    CHECK(endpoint->hostProofKey == std::vector<std::byte>{std::byte{0xEF}});
    CHECK(WaitForSingleObject(unrelatedHandle.Get(), 0) == WAIT_TIMEOUT);

...
void main() {
  group('SessionInvalidatedPayload', () {
    group('methods', () {
      ...
      for (final MapEntry<AdministrativeInvalidationReason, String> entry
        ...
        test('fromJson decodes the wire value "${entry.value}"', () {
      ...
      test(
        'fromJson rejects an unrecognized reason as ProtocolFormatException',
        () {
          expect(
            () => SessionInvalidatedPayload.fromJson(<String, dynamic>{
      ...
      test('fromJson rejects a payload missing the required reason key', () {
        expect(
          () => SessionInvalidatedPayload.fromJson(<String, dynamic>{}),
          throwsA(isA<ProtocolFormatException>()),
        );
      ...
      test(
        'fromJson rejects a payload with the wrong type for the required reason key',
        () {
          expect(
            () => SessionInvalidatedPayload.fromJson(<String, dynamic>{
              'reason': 7,
      ...
      test(
        'fromJson rejects a payload with a null value for the required reason key',
        () {
          expect(
            () => SessionInvalidatedPayload.fromJson(<String, dynamic>{
              'reason': null,

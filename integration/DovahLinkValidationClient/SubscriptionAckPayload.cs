    ...
    IReadOnlyList<string> RejectedStateAreas)
{
    ...
    /// contains a non-string entry.</exception>
    ...
    private static IReadOnlyList<string> DecodeStringArray(JsonObject payload, string key)
    {
        ...
        {
            values.Add(node?.GetValue<string>() ?? throw new FormatException($"{key} entries must be non-null strings."));
        }
        return values;
    }
}

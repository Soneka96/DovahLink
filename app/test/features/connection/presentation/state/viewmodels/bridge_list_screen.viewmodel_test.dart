...
void main() {
  ...
  group('BridgeListScreenViewModel fromStore()', () {
    test('fromStore selects the static default Bridge', () {
      ...
      expect(viewModel.bridges, [Fixtures.buildBridgeEntity()]);
    });

    test(
      'onSelectBridge dispatches ConnectionBridgeSelectedAction, navigating to pairing',
      () {
        final Store<AppState> store = const CreateStore()(
    ...
    test(
      'two ViewModels with the same bridges are equal, regardless of callback identity',
      () {
        final BridgeListScreenViewModel first =
            BridgeListScreenViewModel.fromStore(const CreateStore()());
        final BridgeListScreenViewModel second =
            BridgeListScreenViewModel.fromStore(const CreateStore()());

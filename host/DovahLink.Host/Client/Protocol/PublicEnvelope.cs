...
/// <param name="StateAuthorityId">Identifies the Host's current authoritative-state continuity epoch, per <c>ai/context/protocol/compatibility.md</c>'s state/cache-invalidation section. Required and non-null on <c>hello_ack</c>, <c>state_snapshot</c>, and <c>state_event</c>; <see langword="null"/> (absent on the wire) on every other message.</param>
